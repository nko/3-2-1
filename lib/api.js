...
var Server = function() {
    ...
    httpSrv.get(/\/api\/admin/host(\/[\w\.]+)?\/?/, function(req, res) {
        var host = (req.params[0]) ?
            req.params[0].substring(1, req.params[0].length) :
            undefined;

        if (host) {
            if (!cfg.hostExists(host)) {
                res.writeHead(404);
                res.end();
                return;
            }
            ...
            res.writeHead(200, {'Content-Type' : 'application/json'});
            res.end(JSON.stringify(cfg.getIPsForHost(host)));
        } else {
            var o = {};

            cfg.getHosts().forEach(function(hn) {
                o[hn] = cfg.getIPsForHost(hn);
            });
    ...
    httpSrv.put(/\/api\/admin/host\/([\w\.]+)?\/?/, function(req, res) {
        var host = req.params[0];
        ...
        form.parse(req, function(err, fields, files) {
            ...
            var ips = [];
            fields.ips.split(',').forEach(function(v) {
                if (v.length > 0) {
                    ips.push(v);
                }
            });

            cfg.setHostIPMapping(host, ips);
    ...
    httpSrv.del(/\/api\/admin/host(\/[\w\.]+)?\/?/, function(req, res) {
        var host = (req.params[0]) ?
            req.params[0].substring(1, req.params[0].length) :
            undefined;

        if (host) {
            res.writeHead((cfg.hostExists(host)) ? 200 : 204);
            cfg.removeHost(host);
            res.end();
        } else {
            cfg.getHosts().forEach(function(hn) {

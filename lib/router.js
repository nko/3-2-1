...
Router.prototype._defaultCallback.clientReq = function(req, res) {
    ...
    if (!upstream) {
        ...
        */
        ...
        upstream = {
            host : 'pgriess.github.com',
            port : 80
        };
    }

    // If we're not proxying to an IP address, get rid of the
    // 'Host:' header that the client sent us. It will only
    // confuse the origin.
    if (!upstream.host.match(/^(\d+\.){3}\d+$/)) {

...
var Config = function(proxyMap) {
    ...
    // Get a list of tupeles of (type, rule contents) for
    // the given proxy
    self.getProxyRules = function(proxy) {
        var po = proxyMap[proxy];
        var rules = [];

        po.rules.forEach(function(rn) {
            Object.keys(po.rules).forEach(function(tn) {
                po.rules[tn].forEach(function(r) {
                    rules.push([
                        tn,
                        r.content
                    ]);
